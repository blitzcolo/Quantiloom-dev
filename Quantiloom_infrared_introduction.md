...
**实现说明**：
- 当前版本未包含大气透过率项 $\tau_{\text{atm}}(\lambda)$
- **适用场景**：近地观测（< 100m 距离），大气衰减 < 2%，可忽略
- **遥感应用**：需启用大气校正模块（计划在 v0.1.0，接入 MODTRAN LUT）
- **对比**：VIS 波段在 100m 距离衰减 ~5%，NIR/SWIR 衰减更小（散射系数 λ⁻⁴）
...
1. **太阳反射路径**（line 1564）：
 ...
 ```
 - 使用常数透过率 0.8（MWIR 3-5μm 平均值，海平面，晴空）
 - 实际透过率在 3-5μm 变化范围 0.4-0.95（H₂O 吸收带）
 - 误差影响：太阳分量本身只占 5-20%，总误差 < 2-4%

2. **自发辐射路径**（line 1522-1526）：
 ```hlsl
 L_emission = emissivity * L_blackbody;  // No atmospheric attenuation
 ```
 - 未施加 Beer-Lambert 路径衰减：$\tau = \exp(-\beta \cdot d)$
 - 适用条件：观测距离 < 500m，LWIR 透过率 > 0.95
 - 长距离需要：$L_{\text{sensor}} = L_{\text{emission}} \cdot \tau_{\text{path}} + L_{\text{path}}$

3. **LUT 接口状态**：
 - 数据结构已定义（`AtmosphereTransmittanceLUT.hpp`）
 - Shader 中未激活查询函数（待 v0.1.0 实现）
 - 预留接口：`QueryAtmosphereTransmittance(LUT, λ, h, θ)`

**物理影响与适用场景**：

| 场景 | 距离 | MWIR 透过率 | LWIR 透过率 | 误差估计 | 建议 |
|------|------|-------------|-------------|----------|------|
| 地面监控 | < 100m | 0.98-0.99 | 0.99+ | < 2% | 当前实现可用 |
| 无人机 | 100-500m | 0.92-0.97 | 0.97-0.99 | 3-8% | 启用常数修正 |
| 卫星遥感 | > 10km | 高度变化 | 高度变化 | > 20% | 必须用 LUT |

**改进路径**（Milestone v0.1.0）：
- 接入 MODTRAN 6.0 或 libRadtran LUT
- 实现波长/高度/天顶角三维插值
- 添加路径辐射项（大气自发光贡献）
5.2 各分量详解
...
**波长范围限制**：
- ASTM G-173 标准数据：280-4000 nm
- MWIR 波段需求：3000-5000 nm
- **当前行为**：4000-5000 nm 区间使用**边界值钳制**（4000 nm 的辐照度）

**物理影响**：
- MWIR 末段（4-5 μm）太阳反射分量可能高估 ~10-20%
- 对于 300K 表面，太阳贡献本身只占 5-20%，总误差 < 2-4%

**解决方案**：
- 短期：使用 Planck 5778K 黑体外推（已实现为 fallback）
- 长期：获取扩展到 5000 nm 的实测太阳光谱数据
...
**PSF 实现说明**：
- 当前使用高斯 PSF（$\sigma \propto \lambda \cdot f\# / \text{pixel\_pitch}$），近似 Airy 盘主瓣
- 高斯核可分离卷积，计算效率高（O(N·K) vs O(N·K²)）
- 精确 Airy 盘 PSF（Bessel J₁ 函数）计划在 v0.1.0 实现，用于衍射极限光学系统仿真
...
│         项目           │       当前实现          │         改进方向          │
├────────────────────────┼─────────────────────────┼───────────────────────────┤
│ NIR/SWIR 大气透过率    │ 未实现（τ=1）           │ MODTRAN LUT 查询          │
├────────────────────────┼─────────────────────────┼───────────────────────────┤
│ MWIR 大气透过率        │ 常数 0.8                │ 波长相关 MODTRAN LUT      │
├────────────────────────┼─────────────────────────┼───────────────────────────┤
│ LWIR 路径衰减          │ 未实现                  │ Beer-Lambert exp(-βd)     │
├────────────────────────┼─────────────────────────┼───────────────────────────┤
│ 太阳 MWIR 辐照度       │ LUT 钳制到 4000nm       │ ASTM G-173 扩展到 5μm     │
├────────────────────────┼─────────────────────────┼───────────────────────────┤
│ 传感器 PSF             │ 高斯近似                │ Airy disk（Bessel J₁）    │
├────────────────────────┼─────────────────────────┼───────────────────────────┤
