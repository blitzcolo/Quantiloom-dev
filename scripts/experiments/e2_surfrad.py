...
def convection_from_wind(wind_speed_m_s):
    ...
    for free convection; no stability correction is applied here, which matters
    at night. The "stability" mode below hands the wind to the solver instead
    and lets it apply one.
    """
    return 5.7 + 3.8 * max(0.0, wind_speed_m_s)


def write_forcing(rows, path, mode, sample_minutes=10, convection="material"):
    """The solver's forcing CSV, at a coarser cadence than one minute.
    ...
    `convection` decides how many columns it is and who computes h:

      "material"   eight columns; the solver uses the material's constant
      "wind_h"     nine, the ninth being McAdams h from the measured wind,
                   computed here -- no stability correction anywhere
      "stability"  ten, the ninth zero and the tenth the measured wind speed
                   itself, so the solver applies its own law to it. Same
                   McAdams correlation, damped when the surface is colder than
                   the air, which is the night the ninth column gets wrong.
    """
    if convection not in ("material", "wind_h", "stability"):
        raise ValueError(f"unknown convection mode '{convection}'")
    written = []
    for row in rows:
        ...
        written.append({
            "convection": (convection_from_wind(row["windspd"])
                           if convection == "wind_h" else 0.0),
            "wind_speed": max(0.0, row["windspd"]),
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        elif convection == "stability":
            f.write("# The ninth column is zero, which hands h to the solver, and\n"
                    "#   the tenth is the station's own measured wind speed. The\n"
                    "#   solver applies the same McAdams correlation and damps it\n"
                    "#   where the surface is colder than the air.\n")
        f.write("# time_h, air_k, dni, sun_azimuth_deg, sun_elevation_deg, "
                "sky_k, diffuse_w_m2, relative_humidity"
                + (", convection_w_m2k" if convection != "material" else "")
                + (", wind_speed_m_s" if convection == "stability" else "")
                + "\n")
        for r in written:
            f.write(f"{r['time_h']:.5f}, {r['air_k']:.3f}, {r['dni']:.2f}, "
                    f"{r['azimuth']:.3f}, {r['elevation']:.3f}, {r['sky_k']:.3f}, "
                    f"{r['diffuse']:.2f}, {r['rh']:.1f}"
                    + (f", {r['convection']:.3f}" if convection != "material" else "")
                    + (f", {r['wind_speed']:.3f}" if convection == "stability" else "")
                    + "\n")
...
def write_spec(path, forcing_path, hours, absorptivity, emissivity, nodes=NODE_COUNT,
               interior="adiabatic", convection_model="constant"):
...
def run(args):
    ...
    for days in selected:
        ...
        for emissivity in ([args.emissivity] + args.emissivity_sensitivity):
            ...
            for mode, convection, suffix in (("measured", "material", ""),
                                             ("modelled", "material", ""),
                                             ("measured", "wind_h", "_windh"),
                                             ("measured", "stability", "_stability")):
                tag = f"{mode}{suffix}"
                ...
                write_spec(spec, forcing, written[-1]["time_h"],
                           ...
                           convection_model=("stability" if convection == "stability"
                                             else "constant"))
                ...
                entry["modes"][key] = {
                    "mode": mode, "convection": convection,
                    "wind_driven_h": convection == "wind_h",
