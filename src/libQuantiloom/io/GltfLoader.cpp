...
Material GltfLoader::ParseMaterial(const void* gltfModelPtr, int materialIndex,
    ...
    // ========================================================================
    // Expected glTF extras format:
    //   "extras": {
    //     "quantiloom_material": "Gold_HS111.3B"
    //   }
    // This references a material in the quantiloom_materials.json database
    // generated by SpectralBaker.
    if (!gltfMaterial.extras.Keys().empty()) {
        if (gltfMaterial.extras.Has("quantiloom_material")) {
            ...
            if (qmValue.IsString()) {
                mat.quantiloomMaterialRef = qmValue.Get<std::string>();
                QL_LOG_INFO("  Found Quantiloom material reference: '{}'", mat.quantiloomMaterialRef);
                // Mark as measured since we'll use real spectral data
                mat.spectralSource = Material::SpectralSource::Measured;
            }
        }
    }
