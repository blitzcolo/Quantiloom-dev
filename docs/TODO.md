...
已完成的条目已从本文件移除,记录在 git 历史里:A、B、D、G 组及 C1 C2 C5 E1 E2 F2 H1。
...
### F6. Qt 交互路径的采样种子仍不可复现

**问题** `ExternalRenderContext.cpp:240` 是 `std::mt19937 rng{std::random_device{}()}`,
即 Qt 前端的逐样本种子仍每次运行都不同。CLI 侧同类问题已随 C5 修掉
(`renderer.seed`),这一处没有。

**现状** 交互取景时无所谓,但用 Qt 出图就无法与 CLI 的结果逐位比对,也无法复现自己。
未一并修改的原因是此处无法运行 Qt 前端验证,不想改一段没法验证的代码。

**期待目标** 与 CLI 采用同一套种子约定,或明确记录交互路径不保证可复现。
