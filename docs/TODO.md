...
### C5. 传感器噪声无法设定随机种子,导致测试间歇性失败

**问题** `GenericSensorTest.PRNUAffectsSignalMultiplicatively`
(`test_generic_sensor.cpp:527`)会随机失败。12 次连跑中失败 1 次,
`./build_wsl.sh` 的测试门禁因此也会随机变红。

**现状** 根因不在测试:`GenericSensor.cpp:25` 是
`m_Rng(std::random_device{}())` —— 每次构造都用不可复现的种子,且没有任何接口
可以指定。于是所有 FPN/PRNU 相关测试都是不确定的,该用例只是容差(20%)最紧、
最先暴露的一个,实测偏差 2.008 对上限 2.0,属于擦边。

影响不止于测试:一个号称用于定量验证的渲染器,其传感器噪声无法复现,同一场景两次
渲染的 raw DN 不可能逐位比对。这也和 C3 相冲突 —— 在接入 CI 之前,间歇性红灯会让
CI 信号失去意义。

**期待目标** 噪声可复现:能显式指定种子,默认行为明确;测试门禁不再随机变红。
