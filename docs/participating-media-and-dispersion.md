...
2. ~~**VIS_FUSED 模式做不出色散。**~~ **已解决**：hero 波长采样已实现且无偏（5.5 节）。原先它把折射率钉死在 550 nm，白光原样穿过棱镜；而 RGB 模式反倒有一个三波长近似 —— 光谱模式在光谱现象上不如 RGB 模式。
3. **根因是架构性的**：VIS_FUSED 用**一根光线**携带 32 个波长，而色散意味着每个波长走**不同的几何路径**。解法是 hero wavelength 采样，代价是波长维度的噪声 —— 按设计只出现在色散界面之后。
...
## 5.4 `single` 与 `vis_fused` 的三处分歧（2026-07-30 定位）

两个模式对同一场景给出不同答案，量级随场景内容变化：纯漫反射 **7.9%**，镜面棱镜 **30.9%**。逐项对比 `closesthit.rchit` 两个分支后，差异恰好三处，各自解释一个测量：

| # | 位置 | `vis_fused` | `single` | 谁对 |
|---|---|---|---|---|
| 1 | 自发光 | `ConvertLinearRGBToIlluminantSpectrum(emissive, λ)` (1051) | **`emissive.r`** (1239) | vis_fused |
| 2 | 天空环境光 | `kD·ρ·L_sky` (1046) | **`kD·ρ/PI·L_sky`** (1236) | vis_fused |
| 3 | IBL | `L_ibl`，`useIBL` 时计入 (1056) | **完全没有** (1239) | vis_fused |

**为什么量级不同**：`cornell_box_vis` 的 `sun_radiance` 和 `sky_radiance` 都是 0，整个场景由自发光材质照明 —— 所以只有 #1 生效，得 7.9%。棱镜场景有太阳有天空且 roughness = 0（`useIBL` 为真），#2 #3 同时生效，得 30.9%。

三处都是 `single` 错：

- **#1** 一个 RGB 自发光颜色必须转成光谱，而不是读它的红通道当光谱值。
- **#2** 是重复除 π。两个分支都已经用 `sky_irr / PI` 把辐照度转成了辐亮度；Lambertian 的 `L_out = ρ·E/π` 到此已经完成，`single` 又除了一次，那一项偏小 π 倍。RGB 分支的注释（756–759 行）把这个恒等式写得很清楚。
- **#3** IBL 是真实的光，漏掉就是漏能量。

**这一条值得单独注意**：`single` 正是被标为「定量」（quantitative）的那个模式，光谱分析要用的那个。它现在是三项里错得最多的。而 `vis_fused` 被标为 SPECTRAL。**标签和正确性是反的。**

修 `single` 会改变 `cornell_box_swir/mwir/lwir` 以外所有用 single 模式的结果，需要重新基线化；furnace 判据用的是融合波段，不受影响。

---

## 5.5 hero 波长采样：已完成且无偏（2026-07-30）

VIS_FUSED 现在能色散。实现的是 Wilkie C=1 情形：色散界面上均匀采样一个 λ_h，只追一条携带该波长的光线，由入射面按 `L(λ_h)·cmf(λ_h)/pdf` 还原成颜色，期望值等于确定性栅格所逼近的那个积分。

### 判据

常数 n(λ) 且 **IOR = 1.0**（折射是恒等变换、几何两侧完全相同），hero 必须收敛到确定性栅格：
...
**误差 × √spp 恒定**就是「纯噪声、无偏」。修正前是 0.144 → 0.228 → 0.429（在涨 = 有偏）。均值比也从 1.1786 → 1.0205 → **0.9997**。

### 病因：两处截断，同一类错误

两者都是把一个**必须活到求和/求平均那一步的负中间值**提前削掉：

1. **`raygen.rgen`：逐样本 `clamp(linearHDR, 0.0, 10000.0)`** —— 占 88%。改为对称界。
2. **`ConvertXYZToLinearRGB` 内部 `return max(rgb_linear, 0.0)`** —— 剩下的 2%。改为不截断。

为什么负值是合法的：**窄带或单色光谱必然出 sRGB 色域**，其 R 或 B 通道真的为负。把 λ_h 固定在 590 nm 手算，B = −0.000833；那个负偏移正是在对 λ_h 求平均时与其他波长的正贡献相消的部分。逐样本削掉它，估计器就从「有噪声」变成「有偏」。偏红是因为 XYZ→sRGB 的 R 行系数最大（3.2406），双向摆幅最大。

移除 `ConvertXYZToLinearRGB` 的内部截断对已发布渲染**零影响** —— VIS_FUSED 和 miss 分支都在几行之后把 `output_radiance` 钳到 [0, 1000]，宽带结果的边界和以前一样。只有需要继续参与算术的值（一个波长的贡献，正在被求和的路上）才会到达不截断的调用方。

### 这一类 bug 的普适性

**逐样本非负截断会让任何窄带或单波长的蒙特卡洛估计器产生偏差。** 将来引入 hero C>1、光谱 MIS、或参与介质的谱采样时，同一条都适用。这比 hero 本身更值得记住。

### 排查过程中用到的工具（都已入库）

- **unit-IOR 表**（`assets/data/refractiveindex/unit_ior.yml`）：折射变恒等，把「记账」和「传输」分开 —— 正是它证明偏差不在传输里
- **固定 λ_h**：临时把随机数换成常数，hero 变确定性，可与手算逐通道对照 —— 正是它暴露了 B 被削
- **平坦反射率曲线**：绕开 `ConvertLinearRGBToSpectrum`
- **误差 × √spp**：能识破「偏差冒充噪声」的诊断量。测试脚本里用的判据是「16 倍采样误差应降约 4 倍」

**一个测试自身的陷阱**：check A 一度让参照保持玻璃真实的 IOR、而 hero 用 1.0，等于比较两个不同场景，报出一个不存在的偏差。几何必须两侧一致。

---

## 5.6 想看见色散，比让它算对要难 —— 四次失败的记录

色散的正确性已经验证（5.5 节，无偏）。但做出一张有说服力的光谱图连续失败了四次，每次原因不同，都是物理或架构的，不是参数没调好。记下来，免得重来。

| # | 做法 | 失败原因 |
|---|---|---|
| 1 | 相机沿 −Z 正视棱镜 | 棱镜沿 Z 挤出，光线穿的是两个**平行**的三角端面 —— 等于平板玻璃，净角偏折为零 |
| 2 | 相机沿 −X 侧视 | 几何对了（打在斜面上，偏折 66.3°），但**背景没有对比度**：把一片均匀亮度弯 9.7° 之后还是同一片均匀亮度 |
| 3 | 换 SF11 重火石 + 放大棱镜 | 色度扩散涨了 13.8 倍，但那**全是 hero 的波长噪声** —— 同一玻璃换常数 n 的对照测得 3.63e-04 vs 3.70e-04，真实色散信号只有 0.10% |
| 4 | 狭缝光谱仪（暗室 + 小发光面） | **渲染器对自发光几何没有 NEE**，见下 |

### 第 4 条是架构问题，不是布景问题

自发光只在光线**随机命中**发光面时才有贡献；唯一被显式采样的光源是方向性太阳，它有阴影射线。所以暗室里的一个小发光面是这个渲染器最坏的情形：

- 画面大部分是黑的，多数路径找不到狭缝
- 噪声要几千 spp 才收敛（Studio 里表现为「花屏」）
- 狭缝呈现为一条**不随视角变色的白条** —— 自发光是视角无关的，颜色不变说明看到的是**光源本身**，不是它经棱镜折射的像

最后一条是判定性的：orbit 相机怎么转它都白，就说明那不是折射像，再怎么调取景也没用。

### 下次该怎么做

用这个渲染器**采样得好**的东西：太阳（有显式阴影射线，低噪声）照亮一条**高对比边缘**，让棱镜去展开那条边缘。而不是让路径追踪去随机找一个小发光体。

要做真正的狭缝光谱仪，前提是给面光源加 next-event estimation。

### 一个诊断技巧值得留下

**用「同一材质、常数 n(λ)」做对照**（`assets/data/refractiveindex/flat_n_sf11.yml`）。hero 路径在两边都启用，所以波长噪声相同，两者之差就是纯色散。第 3 次失败正是靠它才发现「色度涨了 13.8 倍」全是噪声 —— 没有这个对照，我会把噪声当成成功。
