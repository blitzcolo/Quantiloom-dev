...
## 5.5 hero 波长的 1.35% 偏差：已定位到形状，未定位到原因

`91059eb` 实现的 hero 路径带一个约 1.35% 的偏差。修完 `single` 三处缺陷后（`e0f0420`），扫频参照在不色散场景上与 `vis_fused` 一致到 1e-6，因此可以逐像素定位。

用**常数 n(λ)**（平的折射率表）：hero 分支照样执行，但什么都不该色散，所以真值就是确定性栅格。hero@2048spp 对栅格：

| 量 | 值 |
|---|---|
| 全图相对误差 | 1.3514% |
| 承载误差的像素 | 1358 / 16384 = **8.29%**，占总误差 **98.9%** |
| 那些像素上的比值 hero/grid | **1.1800** |
| 其余像素上的比值 | **1.0000** |
| 残差分通道（hero − grid） | R +7.13e-4，G +2.68e-4，B +2.38e-4（≈ 3 : 1.1 : 1） |

所以偏差的形状是：**只出现在透射像素上、乘性、约 +18%、偏红，且与色散强度无关**（常数 n 下同样存在）。非透射像素比值精确为 1.0000，说明机制的作用范围是对的，没有泄漏。

**已排除**：参照栅格的求积误差（32 点 vs 128 点只差 0.167%）；`single` 那三处缺陷（修完数字未动）；大气最近邻索引（场景无大气）；Beer-Lambert 通道平均（`attenuationDistance = 0`）；色度校正重复施加（hero 覆盖发生在校正之后，入射面只施加一次）。

**已找到主因（2026-07-30）：`raygen.rgen` 对每个样本做非负截断。**

`linearHDR = clamp(linearHDR, 0.0, 10000.0)` —— 下限 0 逐样本生效。而 hero 光线只携带一个波长，**单色辐亮度转线性 sRGB 会落在色域外，通道合法为负**。把 λ_h 固定在 590 nm 手算验证：

```
hero 渲染值   RGB = [0.01343753, 0.00345765, +0.00012842]
手算期望值    RGB = [0.01412935, 0.00353431, -0.00083283]
```

B 本该是负的。那些负偏移正是在对 λ_h 求平均时与其他波长的正贡献相消的部分，逐样本截断把相消破坏了 —— 于是估计器从「有噪声」变成「有偏」。偏红是因为 XYZ→sRGB 的 R 行系数最大（3.2406），双向摆幅也最大，被截掉的负值最多。

改成对称界 `clamp(..., -10000.0, 10000.0)`。防萤火虫只需要上界，下界为 0 不是必需的；非 hero 样本在 closesthit 和 miss 里已经被钳成非负，所以其他路径不受影响（实测已发布渲染无新增差异）。

**效果**：常数 n、几何相同（IOR = 1.0）下，均值比从 **1.1786 降到 1.0205**，且随采样数完全稳定：

| spp | 128 | 512 | 2048 | 8192 |
|---|---|---|---|---|
| 均值比 | 1.0203 | 1.0204 | 1.0205 | 1.0205 |

**残余 +2.05% 仍未定位**，但它稳定、可复现、比原来小 8.7 倍。已排除：CMF 加权还原（数值检验 hero/grid = 0.999/1.000/0.996，方向也不对）、`ConvertLinearRGBToSpectrum`（换成实测平坦曲线后数字逐位不变）、几何与菲涅尔（IOR = 1.0 下同样存在）、参照栅格求积（0.167%）。

**这个 bug 的普适性值得注意**：任何窄带或单波长的蒙特卡洛估计器都会被逐样本非负截断搞出偏差。将来若引入 hero C>1、光谱 MIS 或参与介质的谱采样，这一条同样适用。
