...
**下一步应查**：`+18% 乘性、R:G:B ≈ 3:1.1:1` 这个指纹。XYZ→sRGB 的 R 行是 `[3.2406, −1.5372, −0.4986]`，对 X 的系数最大，所以「XYZ 被整体高估」会首先表现在 R 上 —— 值得先验证 `E[cmf(λ_h)]·bandwidth` 是否真的等于 `Σcmf(λᵢ)·Δλ`，也就是把 1/pdf 的还原单独拿出来做数值检验，而不是在渲染里检验。CIE LUT 采用等积分归一化（`∫x̄ ≈ ∫ȳ ≈ ∫z̄ ≈ 106.85`），如果 `SampleCIE_XYZ_LUT` 的定义域或步长与这个归一化不自洽，误差就正好是乘性的、且在三通道上按矩阵行权重分配。
